...
% Titel
\node  [name=titel,anchor=north east] at ([xshift=-2cm,yshift=-1.7cm]current page.north east) (komplx) {\fontsize{125}{50}\selectfont Complexity Theory};
...
\node (PAGE_QR) [anchor=south east] at ([xshift=-1.5cm,yshift=1.5cm] current page.south east)  {\includegraphics[height=2.5cm]{qr-code_complexity.pdf}};
\node (POSTERBY) [font=\Large,anchor=south east,align=left] at ([xshift=-0cm]PAGE_QR.south west) {\LARGE Poster by Dr.\,Kai Sauerwald\\(Artificial Intelligence Group, FernUniversität in Hagen)\\\texttt{https://www.kai-sauerwald.de}\\based on a lecture by Prof.\,Dr.\,Schwentick (TU Dortmund University)};
\node (LOGO_AIG) [anchor=south east] at ([xshift=-0.25cm]POSTERBY.south west)  {\includegraphics[height=2.5cm]{AIG.pdf}};
\node (LOGO_FUH) [anchor=south east] at ([xshift=-0.25cm]LOGO_AIG.south west)  {\includegraphics[height=2.5cm]{logo_hagen.pdf}};
...
at ([xshift=-1.5cm,yshift=5cm] current page.south east) 
{
