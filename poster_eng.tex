...
% Titel
\node  [name=titel,anchor=north east] at ([xshift=-1.5cm,yshift=-3.5cm]current page.north east) (komplx) {\fontsize{123}{50}\selectfont Complexity Theory};

\node [font=\normalsize,anchor=north east,align=left] at (komplx.south east) {Poster by Dr. Kai Sauerwald (Artificial Intelligence Group, FernUniversität in Hagen); based on a lecture by Prof. Dr. Schwentick (LogiDAC, TU Dortmund University).};
...
{ 
	\textbf{\textsc{Determinant}} \hfill in $\mathbf{NC^2}$
};
...
{
	\textbf{\textsc{SAT-UNSAT}} \hfill $\mathbf{DP}$-C  \nodepart{second} 
...
\node (littleOverview) [rotate=90, anchor=south west,align=left] at (HORNSAT.north east) {\fontsize{55}{33}\selectfont \textbf{Small}\\\textbf{hierarchy}};
