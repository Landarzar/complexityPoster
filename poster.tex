...
\draw ([yshift=-4cm] 2PTiling.east) [line width=1mm] edge [bend right=25] (EXPTIME.west);
...
at  (2PTiling.south west) 
{
	...
	\nodepart{second} 
	\textbf{Gegeben:} Quantifizierte aussagenlogische Formel $\varphi$ (in Pränex-Normalform und KNF).\newline
	\textbf{Frage:} Ist $K$ erfüllbar?
	\nodepart{third}
...
\filldraw [fill=blue!30] (TSP.south west) -- (TSP.south east) -- (HORNSAT.north east) -- (HORNSAT.north west) -- cycle ;
...
\node (fpt) at ([xshift=5cm,yshift=0cm]Parametrisierung.south west) {$\mathbf{FPT}$};
