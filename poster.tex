...
\node (proofsystem) [rotate=90, anchor=north west] at ([yshift=16cm] SatzAlternation.north west) {
	\fontsize{55}{33}\selectfont \textbf{Beweissysteme}};

\node (npchar) [anchor=south west] at ([xshift=2.5cm,yshift=-1cm]proofsystem.north west)  {$\mathbf{NP}=\mathbf{PCP(log,poly)}=\mathbf{PCP(0,poly)}=\mathbf{PCP(log,1)}$};
...
\node (Parametrisierung) [rotate=90, anchor=north west, rectangle split, rectangle split parts=2, rectangle split part align={left, left}] at ([yshift=21cm] proofsystem.north west) {\fontsize{55}{33}\selectfont \textbf{Beschreibungskompl.} \nodepart{second}\fontsize{55}{33}\selectfont\textbf{\& Parametrisierung}};
