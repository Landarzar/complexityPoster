...
\node (PP) at ([xshift=2cm,yshift=-3cm]PSPACE.south east) {$\mathbf{PP}$};
...
\draw (BPP) [color=black!20,bend right=20,kEdge]edge  (PP);
...
\fill [fill=red!30] (SatzAlternation.north west) -- ([yshift=62cm] SatzAlternation.north west) -- ([yshift=62cm] SatzAlternation.north east) -- (SatzAlternation.north east) -- cycle ;
%%%%%%%%%%%%%%%%
...
\draw [line width=1mm] (PCP.north west)  -- ([yshift=16cm]SatzAlternation.north west);
...
\node (optimierung) [rotate=90, anchor=north west] at ([yshift=17cm] SatzAlternation.north west) {\fontsize{55}{33}\selectfont \textbf{Optimierung}};

\node (fPvsNP) [rotate=90, anchor=north west] at ([xshift=2cm,yshift=17cm] SatzAlternation.north west) {Falls $\mathbf{P\neq NP}$ gilt:};
...
\node (Parametrisierung) [rotate=90, anchor=north west, rectangle split, rectangle split parts=2, rectangle split part align={left, left}] at ([yshift=20cm] optimierung.north west) {\fontsize{55}{33}\selectfont \textbf{Beschreibungskompl.} \nodepart{second}\fontsize{55}{33}\selectfont\textbf{\& Parametrisierung}};
...
\node (paramProbl) [satz, fill=yellow!40,text width=\satzW, anchor=north east]
...
\node (lPD) [anchor=south east] at ([xshift=0cm, yshift=\mydist]lNLD.north east) {$\mathbf{ESO\text{-}Horn, LFP}$};
...
\draw (lAC0) edge [kEdge,loosely dotted] (lAC0D);
\draw (lTC0) edge [kEdge,loosely dotted] (lTC0D);
\draw (lREG) edge [kEdge,loosely dotted] (lREGD);
\draw (lL) edge [kEdge,loosely dotted] (lLD);
\draw (lNL) edge [kEdge,loosely dotted] (lNLD);
\draw (lP) edge [kEdge,loosely dotted] (lPD);
\draw (lNP) edge [kEdge,loosely dotted] (lNPD);
\draw (lPSPACE) edge [kEdge,loosely dotted] (lPSPACED);

% Logiken und Komplexitätsklassen
\node (logikACompl) [satz, fill=yellow!40,text width=\satzW, anchor=north east]
at ([yshift=0cm] lAC0D.south east) 
{
...
% Logiken und Komplexitätsklassen
\node (logikACompl2) [satz,text width=\satzW,minimum width=\satzW, anchor=south east] 
at (SatzAlternation.north east) 
{
	\textbf{Komplexität einer Logik} \nodepart{second} 
	Eine Logik $\mathcal{L}$ hat die Komplexität $\mathcal{C}$, falls für jede geschlossene Formel $\varphi\in\mathcal{L}$ gilt, dass $ \{\text{enc}(\mathfrak{A})\ |\ \mathfrak{A}\mathfrak\models\varphi) \}\in\mathcal{C} $ 
...
\draw [thick,red]([xshift=-1.5cm]current page.east) -- ([xshift=-(1.5cm+33cm)] current page.east);
\fontsize{45}{33}\selectfont
\draw [thick,red] ([xshift=-1.5cm,yshift=-2cm]current page.east) -- ([xshift=-\satzW,yshift=-2cm] current page.east);
...
\fill [fill=blue!50] (TSP.south west) -- (TSP.south east) -- (HORNSAT.north east) -- (HORNSAT.north west) -- cycle ;
