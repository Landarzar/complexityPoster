...
{
	\textbf{\textsc{SAT-FO-String}}  \hfill $\mathbf{EXP}$-C 
	\nodepart{second} 
...
\filldraw [fill=red!30] (SatzAlternation.north west) -- ([yshift=62cm] SatzAlternation.north west) -- ([yshift=62cm] SatzAlternation.north east) -- (SatzAlternation.north east) -- cycle ;
...
\node (Parametrisierung) [rotate=90, anchor=north west, rectangle split, rectangle split parts=2, rectangle split part align={left, left}] at ([yshift=20cm] optimierung.north west) {\fontsize{55}{33}\selectfont \textbf{Beschreibungskomplexität} \nodepart{second}\fontsize{55}{33}\selectfont\textbf{\& Parametrisierung}};

\node (fpt) [anchor=south west] at ([xshift=2cm,yshift=0cm]Parametrisierung.south west) {$\mathbf{FPT}$};
...
\node (paramProbl) [satz, fill=yellow!40,text width=30cm, anchor=north east]
...
\node (lREG) [anchor=center] at ([xshift=-14.5cm]lREGD.east) {$\mathbf{REG}$};
...
\node (ltitel) [anchor=north east] at ([xshift=-.0cm, yshift=-0cm]paramProbl.south east) {\fontsize{55}{33}\selectfont \textbf{Komplexität \& Logik}};
...
% Das Parametrisierte Probleme
\node (logikACompl) [satz, fill=yellow!40,text width=32cm, anchor=north east]
...
\filldraw [fill=blue!50] (TSP.south west) -- (TSP.south east) -- (HORNSAT.north east) -- (HORNSAT.north west) -- cycle ;
